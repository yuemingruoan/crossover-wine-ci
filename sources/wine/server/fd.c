...
{
    struct fd *fd = get_obj_fd( obj );
    struct object *sync_obj = (struct object *)fd->sync;
    struct esync_fd *ret = sync_obj->ops->get_esync_fd( sync_obj, type );
    release_object( fd );
    return ret;
}
...
unsigned int default_fd_get_msync_idx( struct object *obj, enum msync_type *type )
{
    struct fd *fd = get_obj_fd( obj );
    struct object *sync_obj = (struct object *)fd->sync;
    unsigned int ret = sync_obj->ops->get_msync_idx( sync_obj, type );
    release_object( fd );
    return ret;
}
