...
static struct esync_fd *event_sync_get_esync_fd( struct object *obj, enum esync_type *type );
...
static const struct object_ops event_sync_ops =
{
    ...
    event_sync_get_msync_idx,  /* get_msync_idx */
};
...
struct event_sync *create_event_sync( int manual, int signaled )
{
    ...
    if (do_msync())
        event->msync_idx = msync_alloc_shm( signaled, 0 );
