...
static void delete_file( struct device_file *file )
{
    ...
    LIST_FOR_EACH_ENTRY_SAFE( irp, next, &file->requests, struct irp_call, dev_entry )
    {
        ...
        if (do_esync() && file->device->manager && list_empty( &file->device->manager->requests ))
            esync_clear_obj( &file->device->manager->obj );
...
{
    struct device_manager *manager = (struct device_manager *)obj;
    struct object *sync_obj = (struct object *)manager->sync;
    return sync_obj->ops->get_esync_fd( sync_obj, type );
}
...
static unsigned int device_manager_get_msync_idx( struct object *obj, enum msync_type *type )
{
    struct device_manager *manager = (struct device_manager *)obj;
    struct object *sync_obj = (struct object *)manager->sync;
    return sync_obj->ops->get_msync_idx( sync_obj, type );
}
