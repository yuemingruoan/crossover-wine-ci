...
namespace dxvk {
  ...
  void DxvkContext::renderPassBindFramebuffer(
    ...
    if (m_hackRenderAreaTrimmedToViewport) {
      Logger::warn("No attachments, trimming render area to viewport");
      renderArea.extent.width  = std::min(renderArea.extent.width,  uint32_t(ceil(m_state.vp.viewports[0].width)));
      renderArea.extent.height = std::min(renderArea.extent.height, uint32_t(ceil(m_state.vp.viewports[0].height)));
    }
  ...
  void DxvkContext::updateFramebuffer() {
    if (m_flags.test(DxvkContextFlag::GpDirtyFramebuffer) || (m_hackRenderAreaTrimmedToViewport && m_flags.test(DxvkContextFlag::GpDirtyViewport))) {
      m_flags.clr(DxvkContextFlag::GpDirtyFramebuffer);
  ...
  bool DxvkContext::commitGraphicsState() {
    ...
    m_hackRenderAreaTrimmedToViewport = !m_state.om.framebufferInfo.numAttachments() && m_device->adapter()->matchesDriver((DxvkGpuVendor)0, VK_DRIVER_ID_MOLTENVK, 0, 0);
    ...
    m_hackRenderAreaTrimmedToViewport = false;
