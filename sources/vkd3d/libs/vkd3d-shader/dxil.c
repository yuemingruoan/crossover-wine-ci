...
static void register_convert_min_precision(const struct vkd3d_shader_register *const_reg)
{
    ...
    if (reg->data_type == VSIR_DATA_F16)
    {
        reg->data_type = VSIR_DATA_F32;
        reg->precision = VKD3D_SHADER_REGISTER_PRECISION_MIN_FLOAT_16;
    }
    else if (reg->data_type == VSIR_DATA_U16)
    {
        reg->data_type = VSIR_DATA_U32;
    }
}
        ...
        struct vkd3d_shader_message_context *message_context, struct dxbc_shader_desc *dxbc_desc)
{
    ...
    if (!(program->ssa_data_types = vkd3d_calloc(program->ssa_count, sizeof(*program->ssa_data_types))))
    {
        ERR("Failed to allocate data type array.\n");
        vkd3d_shader_error(message_context, &location, VKD3D_SHADER_ERROR_DXIL_OUT_OF_MEMORY,
                "Out of memory emitting SSA data types.");
        return VKD3D_ERROR_OUT_OF_MEMORY;
    }
    for (i = 0; i < sm6->value_count; ++i)
    {
        struct sm6_value *value = &sm6->values[i];
        if (!sm6_value_is_ssa(value))
            continue;
        program->ssa_data_types[value->u.ssa.id] = vsir_data_type_from_dxil(value->type);
    }
