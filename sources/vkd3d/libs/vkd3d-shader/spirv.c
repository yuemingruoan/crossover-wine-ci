...
static uint32_t vkd3d_spirv_build_op_load_aligned(struct vkd3d_spirv_builder *builder,
        uint32_t result_type, uint32_t pointer_id, uint32_t alignment)
{
    if (!alignment)
        ...
        return vkd3d_spirv_build_op_tr2v(builder, &builder->function_stream, SpvOpLoad,
                result_type, pointer_id, SpvMemoryAccessAlignedMask, &alignment, 1);
}
...
static void vkd3d_spirv_build_op_copy_memory(struct vkd3d_spirv_builder *builder,
        uint32_t target_id, uint32_t source_id, uint32_t alignment)
{
    if (!alignment)
        return vkd3d_spirv_build_op2(&builder->function_stream, SpvOpCopyMemory,
                target_id, source_id);
    else
        return vkd3d_spirv_build_op3v(&builder->function_stream, SpvOpCopyMemory,
                target_id, source_id, SpvMemoryAccessAlignedMask, &alignment, 1);
}
        ...
        uint32_t selector, uint32_t default_label, uint32_t *targets, unsigned int target_count)
{
    vkd3d_spirv_build_op2v(&builder->function_stream, SpvOpSwitch,
            selector, default_label, targets, target_count);
}
...
struct spirv_compiler
{
    ...
    struct ssa_register_fwd_fixup *ssa_fwd_fixups;
    size_t ssa_fwd_fixup_capacity;
    size_t ssa_fwd_fixup_count;
};
...
static void spirv_compiler_destroy(struct spirv_compiler *compiler)
{
    ...
    vkd3d_free(compiler->scan_descriptor_info.descriptors);
    vkd3d_free(compiler->ssa_register_info);
    vkd3d_free(compiler->block_label_ids);
    vkd3d_free(compiler->ssa_fwd_fixups);
        ...
        struct vkd3d_shader_message_context *message_context, uint64_t config_flags)
{
    ...
    compiler->scan_descriptor_info.descriptors = NULL;
    compiler->scan_descriptor_info.descriptor_count = 0;
    if (program->descriptors.descriptor_count)
    {
        const struct vkd3d_shader_scan_descriptor_info1 *descriptors = &compiler->program->descriptors;

        if (!(compiler->scan_descriptor_info.descriptors = vkd3d_calloc(descriptors->descriptor_count,
                sizeof(*descriptors->descriptors))))
        {
            spirv_compiler_destroy(compiler);
            return NULL;
        }
        memcpy(compiler->scan_descriptor_info.descriptors, descriptors->descriptors,
                descriptors->descriptor_count * sizeof(*descriptors->descriptors));
        compiler->scan_descriptor_info.descriptor_count = descriptors->descriptor_count;
    }
        ...
        const struct vkd3d_shader_register *reg, struct vkd3d_shader_register_info *register_info)
{
    ...
    {
        struct ssa_register_info *ssa = &compiler->ssa_register_info[reg->idx[0].offset];

        if (!ssa->id)
            ssa->id = vkd3d_spirv_alloc_id(&compiler->spirv_builder);
...
static void spirv_compiler_emit_dereference_register(struct spirv_compiler *compiler,
        const struct vkd3d_shader_register *reg, struct vkd3d_shader_register_info *register_info)
{
    unsigned int component_count, index_count = 0, inbounds_start = UINT_MAX;
    struct vkd3d_spirv_builder *builder = &compiler->spirv_builder;
    ...
    {
        if (reg->idx[reg->idx_count - 1].is_in_bounds)
            inbounds_start = index_count;
    ...
    else if (register_info->is_aggregate)
    {
        ...
        {
            if (reg->idx[1].is_in_bounds)
                WARN("Unhandled in-bounds address attribute.\n");
            indexes[index_count++] = spirv_compiler_emit_register_addressing(compiler, &reg->idx[1]);
        }
        if (reg->idx_count > 1)
        {
            if (reg->idx[0].is_in_bounds)
                WARN("Unhandled in-bounds address attribute.\n");
    ...
    if (index_count)
    {
        ...
        if (!inbounds_start)
            register_info->id = vkd3d_spirv_build_op_in_bounds_access_chain(builder, ptr_type_id,
                    register_info->id, indexes, index_count);
        else
            register_info->id = vkd3d_spirv_build_op_access_chain(builder, ptr_type_id,
                    register_info->id, indexes, index_count);
        register_info->is_access_chain = true;
        if (reg->non_uniform)
        ...
        const struct vkd3d_shader_register_info *reg_info)
{
    ...
    val_id = vkd3d_spirv_build_op_load_aligned(builder, type_id, reg_id, reg->alignment);
...
static void spirv_compiler_append_fwd_fixup(struct spirv_compiler *compiler,
        uint32_t type_id, uint32_t fwd_id, uint32_t actual_id)
{
    struct ssa_register_fwd_fixup *fixup;

    if (!vkd3d_array_reserve((void **)&compiler->ssa_fwd_fixups, &compiler->ssa_fwd_fixup_capacity,
            compiler->ssa_fwd_fixup_count + 1, sizeof(*compiler->ssa_fwd_fixups)))
    {
        ERR("Failed to allocate forward value fixup array.\n");
        spirv_compiler_error(compiler, VKD3D_SHADER_ERROR_SPV_OUT_OF_MEMORY,
                "Failed to allocate forward value fixup array.");
        return;
    }
    fixup = &compiler->ssa_fwd_fixups[compiler->ssa_fwd_fixup_count++];
    fixup->type_id = type_id;
    fixup->fwd_id = fwd_id;
    fixup->actual_id = actual_id;
}

static void spirv_compiler_set_ssa_register_info(struct spirv_compiler *compiler,
        const struct vkd3d_shader_register *reg, uint32_t write_mask, uint32_t val_id)
{
    ...
    if ((fwd_id = compiler->ssa_register_info[i].id))
        spirv_compiler_append_fwd_fixup(compiler, vkd3d_spirv_get_type_id_for_data_type(builder,
                reg->data_type, vsir_write_mask_component_count(write_mask)), fwd_id, val_id);
        ...
        const struct vkd3d_shader_register *reg, uint32_t swizzle, uint32_t write_mask)
{
    ...
    else
    {
        ...
        val_id = vkd3d_spirv_build_op_load_aligned(builder, type_id, reg_info.id, reg->alignment);
        ...
        const struct vkd3d_shader_register *reg, uint32_t write_mask, uint32_t val_id)
{
    ...
    uint32_t write_component_count;
    uint32_t reg_component_count;
    uint32_t type_id;
    ...
    write_component_count = vsir_write_mask_component_count(src_write_mask);
    reg_component_count = vsir_write_mask_component_count(reg_info.write_mask);
    if (reg_info.is_access_chain && write_component_count == 1 && reg_component_count != 1)
    {
        // HACK: 20542: SPIRV-Cross chokes on access chains of access chains when storing tessellation output
        // Work around this by storing the whole vector, so we don't access chain into an individual element
        uint32_t reg_value_id;
        uint32_t write_component = vsir_write_mask_get_component_idx(src_write_mask) - vsir_write_mask_get_component_idx(reg_info.write_mask);
        type_id = vkd3d_spirv_get_type_id(builder, reg_info.component_type, reg_component_count);
        reg_value_id = vkd3d_spirv_build_op_load(builder, type_id, reg_info.id, SpvMemoryAccessMaskNone);
        reg_value_id = vkd3d_spirv_build_op_composite_insert1(builder, type_id, val_id, reg_value_id, write_component);
        vkd3d_spirv_build_op_store(builder, reg_info.id, reg_value_id, SpvMemoryAccessMaskNone);
        return;
    }
...
static void spirv_compiler_emit_interpolation_decorations(struct spirv_compiler *compiler, bool is_output,
        enum vkd3d_shader_component_type component_type, uint32_t id, enum vkd3d_shader_interpolation_mode mode)
{
    ...
    switch (mode)
    {
        case VKD3DSIM_NONE:
            ...
            if (is_output || compiler->shader_type != VKD3D_SHADER_TYPE_PIXEL || component_type == VKD3D_SHADER_COMPONENT_FLOAT)
                break;
            /* fall through */
        ...
        enum vkd3d_shader_register_type reg_type, unsigned int element_idx)
{
    ...
    else
    {
        ...
        spirv_compiler_emit_interpolation_decorations(compiler, false, component_type, input_id,
        ...
        enum vkd3d_shader_register_type reg_type, unsigned int element_idx)
{
    ...
    else
    {
        ...
        spirv_compiler_emit_interpolation_decorations(compiler, true, component_type, id,
                signature_element->interpolation_mode);
    }
        ...
        const struct vkd3d_shader_instruction *instruction)
{
    ...
    {
        bool is_uint64 = src[0].reg.data_type == VSIR_DATA_U64;
        /* In D3D bits are numbered from the most significant bit. */
        component_count = vsir_write_mask_component_count(dst->write_mask);
        uint_max_id = is_uint64 ? spirv_compiler_get_constant_uint64_vector(compiler, UINT64_MAX, component_count)
                : spirv_compiler_get_constant_uint_vector(compiler, UINT32_MAX, component_count);
        ...
        rev_val_id = vkd3d_spirv_build_op_isub(builder, type_id,
                spirv_compiler_get_constant_uint_vector(compiler, is_uint64 ? 63 : 31, component_count), val_id);
        ...
        const struct vkd3d_shader_instruction *instruction)
{
            ...
            && vkd3d_swizzle_is_equal(dst_reg_info.write_mask, src->swizzle, src_reg_info.write_mask))
    {
        ...
        vkd3d_spirv_build_op_copy_memory(builder, dst_id, src_id, src->reg.alignment);
        return;
    }
            ...
            && dst_reg_info.write_mask == VKD3DSP_WRITEMASK_ALL)
    {
        ...
        val_id = vkd3d_spirv_build_op_load_aligned(builder, type_id, src_id, src->reg.alignment);
        dst_val_id = vkd3d_spirv_build_op_load(builder, type_id, dst_id, SpvMemoryAccessMaskNone);
        ...
        const struct vkd3d_shader_instruction *instruction)
{
    struct vkd3d_spirv_builder *builder = &compiler->spirv_builder;
    const struct vkd3d_shader_src_param *src = instruction->src;
    bool is_64_bit = src[0].reg.data_type == VSIR_DATA_U64;
    unsigned int i, j, case_count, word_count;
    ...
    case_count = instruction->src_count - 3;
    word_count = case_count + (is_64_bit ? case_count / 2u : 0);
    ...
    src = &src[3];
    for (i = 0, j = 0; i < case_count; i += 2)
    {
        cases[j++] = src[i].reg.u.immconst_u32[0];
        if (is_64_bit)
            cases[j++] = src[i].reg.u.immconst_u64[0] >> 32;
        cases[j++] = spirv_compiler_get_label_id(compiler, src[i + 1].reg.idx[0].offset);
    }

    vkd3d_spirv_build_op_switch(builder, val_id, default_id, cases, j);
        ...
        const struct vkd3d_shader_src_param *src1, uint32_t src1_mask)
{
    ...
    if (offset->reg.alignment)
        WARN("Ignoring alignment %u.\n", offset->reg.alignment);
...
{
    const struct vkd3d_shader_scan_descriptor_info1 *descriptors = &compiler->scan_descriptor_info;
    unsigned int i;
...
#ifdef _WIN32
static bool match_program_name(const char *query)
{
    char exe_path[_MAX_PATH];
    const char *name;

    GetModuleFileNameA(NULL, exe_path, _MAX_PATH);

    if ((name = strrchr(exe_path, '/')) || (name = strrchr(exe_path, '\\')))
        ++name;
    else
        name = exe_path;

    return !ascii_strcasecmp(name, query);
}
#else
static bool match_program_name(const char *query)
{
        ...
        const struct vkd3d_shader_compile_info *compile_info, struct vkd3d_shader_code *spirv)
{
    ...
    unsigned int i, j, max_element_count;
    enum vkd3d_result result = VKD3D_OK;
    ...
    if (program->ssa_count)
    {
        ...
        if (program->ssa_data_types)
        {
            for (i = 0; i < program->ssa_count; ++i)
                compiler->ssa_register_info[i].data_type = program->ssa_data_types[i];
        }
    }
    ...
    for (ins = vsir_program_iterator_head(&it); ins && result >= 0; ins = vsir_program_iterator_next(&it))
    {
        ...
        if (result < 0 || (it.idx < it.array->count - 1 && it.array->elements[it.idx + 1].opcode == VSIR_OP_PHI))
            continue;
        for (j = 0; j < compiler->ssa_fwd_fixup_count; ++j)
        {
            struct ssa_register_fwd_fixup *fixup = &compiler->ssa_fwd_fixups[j];
            vkd3d_spirv_build_op3(&builder->function_stream, SpvOpCopyObject, fixup->type_id,
                    fixup->fwd_id, fixup->actual_id);
        }
        compiler->ssa_fwd_fixup_count = 0;
    }
        ...
        struct vkd3d_shader_code *out, struct vkd3d_shader_message_context *message_context)
{
    ...
    if (program->shader_version.major >= 6
            && !spirv_compiler_is_target_extension_supported(spirv_compiler,
                    VKD3D_SHADER_SPIRV_EXTENSION_EXT_DESCRIPTOR_INDEXING)
            && match_program_name("Diablo IV.exe")
            && (ret = vkd3d_shader_normalise_descriptor_arrays(&program->instructions, compile_info,
                    &spirv_compiler->scan_descriptor_info, spirv_compiler->message_context)) < 0)
    {
