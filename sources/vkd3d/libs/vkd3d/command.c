        ...
        VkQueueFlags vk_queue_flags, struct d3d12_resource *after, VkImageMemoryBarrier *vk_barrier)
{
    ...
    if (!(format = vkd3d_format_from_d3d12_resource_desc(device, &after->desc, 0)))
    {
        ERR("Resource %p has invalid format %#x.\n", after, after->desc.Format);
        return;
    }
    ...
    if ((after->desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_SIMULTANEOUS_ACCESS) || after->desc.Layout == D3D12_TEXTURE_LAYOUT_ROW_MAJOR)
        vk_barrier->newLayout = VK_IMAGE_LAYOUT_GENERAL;
    else if (after->desc.Flags & D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE)
        vk_barrier->newLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    else if (after->desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL)
        vk_barrier->newLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL;
    else
        vk_barrier->newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    vk_barrier->srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    vk_barrier->dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    vk_barrier->image = after->u.vk_image;
    ...
    for (i = 0; i < barrier_count; ++i)
    {
        ...
        switch (current->Type)
        {
            ...
            case D3D12_RESOURCE_BARRIER_TYPE_ALIASING:
            {
                ...
                if ((before && (before->flags & VKD3D_RESOURCE_DEDICATED_HEAP)) || (after && (after->flags & VKD3D_RESOURCE_DEDICATED_HEAP)))
                    continue;
                ...
                if (after && d3d12_resource_is_texture(after))
                {
                    ...
                    VK_CALL(vkCmdPipelineBarrier(list->vk_command_buffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0,
                            0, NULL, 0, NULL, 1, &vk_alias_barrier));
                }
                else
                {
                    VkMemoryBarrier vk_alias_barrier;
                    if (!after)
                    ...
                    vk_alias_barrier.sType = VK_STRUCTURE_TYPE_MEMORY_BARRIER;
                    vk_alias_barrier.pNext = NULL;
                    vk_alias_barrier.srcAccessMask = alias_src_access;
                    vk_alias_barrier.dstAccessMask = alias_dst_access;
                    VK_CALL(vkCmdPipelineBarrier(list->vk_command_buffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0,
                            1, &vk_alias_barrier, 0, NULL, 0, NULL));
                }
                continue;
            }
