    ...
    def _begin_span(self, style, tags=()):
        ...
        for item in style.split(';'):
            ...
            try:
                ...
                raise ValueError("the '%s' style is malformed" % item) # pylint: disable=W0707
    ...
    def startElement(self, name, attrs):
        ...
        if name == 'br':
            ...
            if pixbuf is not None:
                ...
                self._insert_text("[IMG: %s]" % alt) # pylint: disable=E0601
...
class HtmlTextView(Gtk.TextView):
    ...
    def _draw(self, widget, cr):
        ...
        Gtk.render_background(style, cr, 0, 0,
                              widget.get_allocated_width(),
                              widget.get_allocated_height())

        # The widget initially gets assigned too much space, so make
        # sure we recompute that, or else things may fall outside of
        # the window.
        self.queue_compute_expand()
    ...
    def display_html(self, html):
        ...
        if not html.startswith('<!DOCTYPE'):
            html = '<!DOCTYPE body [ ' + \
                   ...
                   '<!ENTITY trade "&#x2122;"> ' + \
                   ']>' + html
        if not isinstance(html, type('')) and isinstance(html, type('')):
